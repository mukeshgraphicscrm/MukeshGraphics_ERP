...
export const generateOrdersListPDF = async (orders, customers, products, fromDate, toDate, exportType = 'pdf') => {
  const doc = new jsPDF('landscape');
  
  const brandPrimary = [30, 58, 138];
  const textPrimary = [51, 65, 85];
  ...
  doc.setFont("helvetica", "bold");
  doc.text("MUKESH GRAPHICS", 14, 22);
  ...
  if (fromDate && toDate) {
    dateText = `Date Range: ${new Date(fromDate).toLocaleDateString('en-IN')} to ${new Date(toDate).toLocaleDateString('en-IN')}`;
  } else if (fromDate) {
    dateText = `From: ${new Date(fromDate).toLocaleDateString('en-IN')}`;
  } else if (toDate) {
    dateText = `To: ${new Date(toDate).toLocaleDateString('en-IN')}`;
  }
  doc.setFontSize(10);
  ...
  // Table Data
  const tableColumn = ["Order No.", "Customer", "Product", "Quantity", "Amount", "Order Date", "Delivery Date", "Status"];
  const tableRows = [];

  orders.forEach(order => {
    const c = customers[order.customerId];
    const customerName = c ? (c.brandName || c.name) : 'DELETED CUSTOMER';
    
    let productName = '';
    if (Array.isArray(order.productId)) {
      productName = order.productId.map(id => products[id]?.name || id).join('\n');
    } else {
      productName = products[order.productId]?.name || order.productId;
    }

    let quantity = '';
    if (Array.isArray(order.productId) && order.quantities) {
      quantity = order.productId.map(id => (order.quantities[id] || '0').toString()).join('\n');
    } else {
      quantity = order.quantity ? order.quantity.toLocaleString('en-IN') : '0';
    }

    let amount = '';
    if (Array.isArray(order.productId) && order.amounts) {
      amount = order.productId.map(id => `Rs. ${order.amounts[id] || '0'}`).join('\n');
    } else {
      amount = `Rs. ${order.amount ? order.amount.toLocaleString('en-IN') : '0'}`;
    }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Generated by Mukesh Graphics ERP | Page ${i} of ${pageCount}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 10, { align: 'center' });
  }
